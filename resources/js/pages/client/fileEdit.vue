...
function submit(){
    form.put(`/file-view/${document.value.id}`,{
        forceFormData:true,
        preserveScroll:true,
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs"  class="relative">
        ...
        <RecordForm>
            <RecordFormHeader :title-form="`Actualizar ${document.type_document }`" return-url="/my-files"/>
            <RecordFormBody  :handle="submit">

                <div class="grid gap-1">
                    <Label for="file">{{document.type_document}}</Label>
                    <Input
                        id="file"
                        type="file"
                        accept=".pdf"
